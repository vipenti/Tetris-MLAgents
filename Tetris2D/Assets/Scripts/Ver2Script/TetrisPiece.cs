...
public class TetrisPiece : MonoBehaviour
{
    ...
    private Dictionary<float, Vector2Int[]> segmentPositions;

    public void Init(TetrisGame controller, Grid grid, bool calculatePositions = false)
    {
        ...
        {
            segmentPositions = new Dictionary<float, Vector2Int[]>();
    ...
    public void CalculatePositions(float angle)
    {
        ...
        Vector2Int[] positions = new Vector2Int[len];
        ...
        for (int i = 0; i < segments.Count; i++)
        {
            ...
            positions[i] = new Vector2Int(Mathf.RoundToInt(rotatedPosition.x), Mathf.RoundToInt(rotatedPosition.y));
        }
    ...
    private void Update()
    {
        ...
        if (Time.time - previousTime > Settings.FallTime)
        {
            if (!MoveIfValid(0, -1))
            {
                if (!CheckForGameOver())
    ...
    public void SetBlockPositions(Vector2Int[] positions)
    {
        int i = 0;
    ...
    public bool MoveIfValid(int x, int y = 0)
    {
        foreach (Transform child in transform)
        {
            ...
            // outside bounds
            if (roundedX < 0 || roundedX >= Settings.GridWidth || roundedY < 0 || roundedY >= Settings.GridHeight) return false;
