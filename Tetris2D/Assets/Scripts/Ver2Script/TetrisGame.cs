...
public class TetrisGame : MonoBehaviour
{
    ...
    public void Init(TetrisAgent agent)
    {
        ...
        gridTemp = new int[Settings.GridWidth, Settings.GridHeight];
        //ui.SetHighScore(0, 0);
    ...
    public void BlockPlaced(GridState state)
    {
        ...
        {
            agent.AddReward(Settings.Reward.BlockPlaced);
        }
    ...
    {
        currentPoints += Settings.Points[lines - 1];
        totalLines += lines;
    ...
    {
        UpdateStats();
        agent.AddReward(Settings.Reward.GameOver);
        agent.EndEpisode();
    }

    public void CreateBlock(int x, float rotation)
    {
        Vector2Int[] positions = tetrominoes[currentPiece].GetBlockPositions(x, Settings.SpawnY, rotation);
    ...
    private async void GetNextStates()
    {
        ...
        //if (agent.MaskedActions.Count >= Settings.PossibleStates)
        {
            GameOver();
        }
    ...
    private void PopulateStates()
    {
        ...
        {
            for (int j = 0; j < Settings.Rotations.Length; j++)
            {
    ...
    public float[] GetState(int piece, int x, int rot)
    {
        ...
        Vector2Int[] p0Positions = tetrominoes[piece].GetBlockPositions(x, Settings.SpawnY, Settings.Rotations[rot]);
        ...
        if (grid.CheckPositionsAreValid(p0Positions, gridTemp))
        {
            ...
            states[1] = states[1] / Settings.GridSize; // sum height
            states[2] = states[2] / Settings.GridSize; // bumpiness
            states[3] = states[3] / Settings.GridSize; // numHoles
        }
        else
        {
            states[0] = -1;
