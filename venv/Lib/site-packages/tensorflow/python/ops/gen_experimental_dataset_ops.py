...
def register_dataset(dataset, address, protocol, external_state_policy, element_spec="", name=None):
  ...
  if tld.is_eager:
    ...
    try:
      return register_dataset_eager_fallback(
          ...
          element_spec=element_spec, name=name, ctx=_ctx)
    except _core._SymbolicException:
      pass  # Add nodes to the TensorFlow graph.
  # Add nodes to the TensorFlow graph.
